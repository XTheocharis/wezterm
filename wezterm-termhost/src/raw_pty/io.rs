//! Pipe and handle helpers for the raw_pty adapters.
//!
//! `create_anon_pipe` — anonymous pipe with per-end inheritance control.
//! `dup_handle` — wraps `DuplicateHandle`.
//! `HandleReader` / `HandleWriter` — `std::io` wrappers over raw handles.
...
pub(crate) fn dup_handle(src: HANDLE) -> Option<RawHandle> {
    // DuplicateHandle creates a new kernel handle for the same object;
    // the original remains valid. Both must be closed independently.
