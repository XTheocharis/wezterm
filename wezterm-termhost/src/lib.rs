...
/// Future returned by host-injected spawn callbacks.
pub type SpawnFuture = Pin<Box<dyn Future<Output = ()> + 'static>>;

/// Host functions injected by the GUI binary. The crate calls these
/// instead of depending on `wezterm-gui` directly.
pub struct HostFunctions {
    pub spawn_fallback_tab: fn() -> SpawnFuture,
    pub trigger_gui_attached: fn(domain_id: usize) -> SpawnFuture,
    pub cell_pixel_dims: fn(dpi: f64) -> anyhow::Result<(usize, usize)>,
}

static HOST: OnceLock<HostFunctions> = OnceLock::new();

pub(crate) fn host() -> &'static HostFunctions {
    HOST.get().expect("host functions not set")
}
...
pub fn install(host: HostFunctions) -> Option<TermHostState> {
    HOST.set(host).ok().expect("host functions already set");
...
async fn spawn_default_tab_in_new_window() -> anyhow::Result<()> {
    ...
    let _tab = domain
        .spawn(
            config.initial_size(dpi as u32, Some((host().cell_pixel_dims)(dpi)?)),
            None,
            None,
            window_id,
        )
        .await?;
    (host().trigger_gui_attached)(domain.domain_id()).await;
    Ok(())
}
...
pub fn await_handoff() {
    promise::spawn::spawn(async move {
        ...
        if !HANDOFF_RECEIVED.load(Ordering::SeqCst) {
            // Awaited (not fire-and-forget) so the Activity guard stays
            // alive until the spawn completes. If it fails, the guard
            // drops, mux fires Empty, process exits.
