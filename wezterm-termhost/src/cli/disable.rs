use crate::{key_is_wezterm_owned, TERMHOST_HANDOFF_IIDS, WEZTERM_PROXY_STUB_CLSID};
...
impl DisableCommand {
    pub fn run() -> anyhow::Result<()> {
        ...
        // cleanup is done.
        let restore_result = crate::registration::restore_delegation_backup();
...
fn unregister_openconsole_fallback() {
    ...
    let clsid = crate::WEZTERM_TERMHOST_FALLBACK_CONSOLE_CLSID;
    let key_path = crate::registration::clsid_registry_path(clsid);
    let hkcu = RegKey::predef(HKEY_CURRENT_USER);
...
mod tests {
    ...
    fn restore_happy_path() {
        ...
        let restored = crate::registration::restore_delegation_backup().unwrap();
        assert!(restored);
    ...
    fn interloper_protection_console_only_changed() {
        ...
        let terminal: String = startup.get_value(DELEGATION_TERMINAL).unwrap();
        assert_eq!(terminal, crate::WEZTERM_TERMHOST_TERMINAL_CLSID);
    ...
    fn clear_removes_both_values() {
        ...
        crate::registration::clear_delegation_backup().unwrap();
