//! The `EstablishPtyHandoff` COM method that receives a new ConPTY session.
//!
//! This is the callback Windows invokes on the `Instance` object when a
//! terminal handoff occurs. The method receives the raw pipe and process
//! handles from Windows and forwards them to the WezTerm callback
//! registered via `set_callback` in the parent module.
...
use winapi::shared::winerror::{E_FAIL, E_POINTER, E_UNEXPECTED, S_OK};
...
pub(super) unsafe extern "system" fn establish_pty_handoff(
    ...
    // Zero the [out] parameters first. In COM, an [out] parameter is one
    // the callee writes for the caller to read back; an HRESULT is a
    // 32-bit COM return code (S_OK = success). If we return a failure
    // HRESULT without writing the [out] params, a non-compliant caller
    // could read stale (and potentially dangerous) handle values from
    // the caller's stack.
...
mod tests {
    ...
    use super::*;
    use crate::handoff::instance::VTABLE;
    use crate::handoff::take_singleton;
    use crate::raw_pty::create_anon_pipe;
