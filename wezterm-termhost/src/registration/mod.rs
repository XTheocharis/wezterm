//! Read/write the Windows Registry keys that control the default
//! terminal selection.
//!
//! The Registry is Windows' hierarchical configuration database (a
//! tree of key-value pairs). At console startup, conhost reads
//! `HKCU\Console\%%Startup\DelegationConsole` / `DelegationTerminal`
//! to find the terminal to delegate to
//! (`microsoft/terminal/src/propslib/DelegationConfig.cpp:233-286`).
//!
//! HKCU (HKEY_CURRENT_USER) is the per-user section. We never write to
//! HKLM (HKEY_LOCAL_MACHINE, system-wide) because doing so can clobber
//! Windows Terminal's MSIX-managed registrations and break console apps.
...
/// The OpenConsole CLSID from Windows Terminal Release (see
...
/// Registry value name that marks entries WezTerm owns. We check this
/// before modifying any registry entry so we never clobber or unregister
/// entries owned by another terminal host.
pub(crate) const WEZTERM_OWNED_VALUE: &str = "WezTermOwned";

/// Backup value names stored next to the Delegation values. They save the
/// previous terminal selection so `disable` can restore it.
...
/// True if both Delegation values match WezTerm's CLSIDs. We check both
/// because a partial match is ambiguous — restoring from partial state
/// would clobber another host's console selection.
...
/// Read the current DelegationConsole and DelegationTerminal values from
/// the registry, or return `None` if neither is set.
...
/// Save the current DelegationConsole/DelegationTerminal values to
/// WezTerm_Last_* backup entries, so `disable` can restore them later.
///
/// Idempotent: skips if WezTerm is already the default (preserves the
/// original pre-WezTerm backup). Missing values are saved as the null
/// GUID `{0000...}` (meaning "let Windows decide").
...
/// Restore DelegationConsole/DelegationTerminal from the WezTerm_Last_*
/// backup values. Returns `true` on restore, `false` if skipped (current
/// default is not WezTerm — interloper protection). Always clears the
/// backup values afterward.
...
/// Delete the WezTerm_Last_Console and WezTerm_Last_Terminal backup values.
...
/// Read a CLSID's `LocalServer32` registry value (the exe path for a
/// COM class) from a registry root (HKCU or HKLM), extract just the exe
/// path (stripping quotes and arguments), and return it as a `PathBuf`.
/// Returns `None` if missing or empty.
pub fn read_local_server_exe(root: &RegKey, clsid: &str) -> Option<PathBuf> {
    let path = format!("{}\\LocalServer32", clsid_registry_path(clsid));
