//! Register our bundled OpenConsole.exe as the ConPTY host for handoff.
//! This fallback ensures handoff works on systems without Windows Terminal.
...
/// Find the bundled OpenConsole.exe next to the current exe, or in a
/// cargo `target/{debug,release}` directory above it (for dev builds).
...
/// Register the bundled OpenConsole.exe under its CLSID in HKCU.
/// Skips if a foreign server is already registered (ownership check).
/// Refreshes the path if our own registration points at a stale location.
pub fn register_openconsole_fallback() -> anyhow::Result<()> {
    ...
    // If a server is already registered, check ownership. Only refresh our
    // own entries (marked with WezTermOwned) when the path has changed
    // (e.g. install moved or upgraded). Foreign entries are left alone.
...
/// Read the LocalServer32 path we wrote under this CLSID in HKCU.
/// Returns `None` if the key is absent, lacks our ownership marker, or
/// has no readable LocalServer32 value.
