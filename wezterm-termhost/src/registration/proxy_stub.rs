//! Register the proxy/stub DLL (OpenConsoleProxy.dll) for cross-process
//! COM marshalling. A proxy stub serializes COM interface calls across
//! process boundaries — the handoff interfaces need this because the
//! terminal host and conhost run in separate processes.
...
// This CLSID must match the PROXY_CLSID_IS baked into OpenConsoleProxy.dll
// at compile time (microsoft/terminal Host.Proxy.vcxproj). We reuse the
// Windows Terminal Stable channel CLSID — safe because MSIX packaged-COM
// (used by WT) and classic HKCU COM (used by WezTerm) resolve through
// independent subsystems.
pub const WEZTERM_PROXY_STUB_CLSID: &str = "{3171DE52-6EFA-4AEF-8A9F-D02BD67E7A4F}";

/// Interface IIDs (COM interface identifiers) that need proxy/stub
/// marshalling (`microsoft/terminal/src/host/proxy/*.idl`):
...
/// Register the proxy/stub DLL in HKCU (per-user only).
///
/// We always register our own stub even when Windows Terminal is
/// installed. MSIX packaged-COM and classic HKCU COM are independent
/// subsystems, and some Windows 10 builds fail to resolve packaged-COM
/// classes for LocalServer32 activation.
///
/// Skips entries that already have non-empty values and aren't ours.
