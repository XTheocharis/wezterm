...
//!
//! `start_listening` registers the factory on the caller's thread (which
//! must have initialized COM under the STA model — all handoff calls
//! arrive on this one thread).
//!
//! The returned `HandoffGuard` revokes the registration on drop. Drop
//! order matters: `HandoffGuard` must drop before `CoinitGuard` so
//! revocation happens while COM is still active.
...
/// outlive this guard so `CoRevokeClassObject` runs while COM is still
/// initialized.
