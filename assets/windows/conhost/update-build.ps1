...
<#
.SYNOPSIS
    Build OpenConsole.exe, conpty.dll, and OpenConsoleProxy.dll from
    microsoft/terminal source, producing WezTerm-compatible binaries.

.DESCRIPTION
    This script compiles Microsoft's open-source Windows Terminal code to
    produce the three binaries WezTerm bundles as a ConPTY host.

    It clones microsoft/terminal at the requested tag, restores NuGet
    packages, and builds 3 MSBuild projects with
    WindowsTerminalBranding=Release so OpenConsoleProxy.dll embeds the
    correct COM class identifier ({3171DE52-...}).

    Requires Visual Studio 2022+ with: VCTools workload, VC.Tools.x86.x64,
    Windows11SDK.26100, VC.ATL. Idempotent: skips the clone step when the
    source tree is already at the requested tag.
...
if (Test-Path $Src) {
    if (-not (Test-Path (Join-Path $Src '.git'))) {
        # If a previous clone failed, $Src exists without .git/. The next
        # `git clone` would refuse because the directory isn't empty. Throw
        # a clear error so the user knows to delete $Src and retry.
        throw "$Src exists without .git (partial clone?). Remove $Src and re-run."
    }
        # Verify HEAD is at the requested tag. This prevents silently building
        # the wrong version after a tag switch. We don't auto-fetch a new tag
        # into an existing shallow clone — delete $Src and re-run to switch tags.
...
Step 4 "Building 3 target projects ($Configuration $Platform, WindowsTerminalBranding=Release)"
        # Use forward slashes in SolutionDir to work around an MSBuild arg-parsing
        # bug (MSBuild #3964). When $Src contains spaces, PowerShell auto-quotes
        # the argument, and a trailing backslash before the closing quote escapes
        # the quote. MSBuild and Windows both accept forward slashes.
$Common = '/t:Rebuild',
...
foreach ($name in $Targets.Keys) {
    ...
    Write-Host "`n--- Build $i/$($Targets.Count): $name ---"
        # Use try/finally so Pop-Location runs even if MSBuild fails to start.
        # Without this, a build failure would leave the shell in the wrong directory.
    Push-Location $Src
    try {
...
foreach ($n in $Targets.Keys) {
    if (-not (Test-Path (Join-Path $Bin $n))) { throw "Missing: $Bin\$n" }
        ...
        # A wrong CLSID means the DLL can't be used for defterm — the COM runtime
        # won't find the right marshalling code. This is a hard error, not a warning.
...
Write-Host ' SUCCESS - 3 artifacts built' -ForegroundColor Green
        # Only list the three files we built, so stale files from previous builds
